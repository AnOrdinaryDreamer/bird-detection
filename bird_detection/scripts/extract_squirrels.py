...
def extract_from_source(
    ...
    for label_file in label_dir.glob("*.txt"):
        ...
        with (dest_label_dir / label_file.name).open("w", encoding="utf-8") as out_f:
            out_f.write("\n".join(selected) + "\n")
        ...
        for line in selected:
            ...
            width_px = x2 - x1
            height_px = y2 - y1
            pixel_lines.append(f"{parts[0]} {x1:.2f} {y1:.2f} {width_px:.2f} {height_px:.2f}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract YOLO labels for a target class and export YOLO + pixel formats."
    )
    parser.add_argument(
        "--source-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Destination directory for the filtered subset (images/, labels/, labels_pixel/).",
    )
    parser.add_argument(
        "--class-id",
        type=str,
    ...
    dest_label_dir = args.dest_root / "labels"
